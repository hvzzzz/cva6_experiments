...
char *argv[];
{
    ...
    if (fork()) { /* parent process */
        ...
        close(p2[1]);
        while (1) {
            if ((ret = write(p1[1], (char *)&iter, sizeof(iter))) !=
                sizeof(iter)) {
            ...
            if ((ret = read(p2[0], (char *)&check, sizeof(check))) !=
                sizeof(check)) {
        ...
        close(p2[0]);
        while (1) {
            if ((ret = read(p1[0], (char *)&check, sizeof(check))) !=
                sizeof(check)) {
                if ((ret == 0)) { /* end-of-stream */
                    alarm(0);
                    report(); /* does not return */
            ...
            if ((ret = write(p2[1], (char *)&iter, sizeof(iter))) !=
                sizeof(check)) {
                if ((ret == -1) && (errno == EPIPE)) {
                    alarm(0);
                    report(); /* does not return */
