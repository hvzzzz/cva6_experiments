...
char *argv[];
{
...
#ifndef UNIXBENCH
    if (getinput) {
        printf("Enter the following which will be added with results to file "
               "WHETS.RES\n");
        printf("When submitting a number of results you need only provide "
               "details once\n");
        printf("but a cross reference such as an abbreviated CPU type would be "
               "useful.\n");
        printf("You can kill (exit or close) the program now and no data will "
               "be added.\n\n");
    ...
    for (section = 1; section < 9; section++) {
        fprintf(outfile, "%s  %24.17f   ", headings[section], results[section]);
    ...
    fprintf(outfile, " %9.3f %9.3f %9.3f", mwips, loop_mflops[1],
            loop_mflops[2]);
    fprintf(outfile, " %9.3f %9.3f %9.3f", loop_mflops[6], loop_mops[5],
            loop_mops[8]);
    fprintf(outfile, " %9.3f %9.3f %9.3f\n\n", loop_mops[4], loop_mops[3],
            loop_mops[7]);

    fclose(outfile);

    printf("\n");
    printf("A new results file will have been created in the same directory as "
           "the\n");
    printf(".EXE files if one did not already exist. If you made a mistake on "
           "input, \n");
    printf("you can use a text editor to correct it, delete the results or "
           "copy \n");
    printf("them to a different file name. If you intend to run multiple tests "
           "you\n");
    printf(
...
void whetstones(long xtra, long x100, int calibrate) {
    ...
    timeb = dtime() - timea;
    x = e1[0] + e1[1];
    pout("N4 fixed point   \0", (float)(n4 * 15) * (float)(xtra), 2, x, timeb,
         calibrate, 4);
    ...
    timeb = dtime() - timea;
    pout("N5 sin,cos etc.  \0", (float)(n5 * 26) * (float)(xtra), 2, y, timeb,
         calibrate, 5);
