...
#pragma omp for nowait schedule(static)
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      for (m = 0; m < 5; m++) {
        v[i][j][k][m] =
            v[i][j][k][m] - omega * (ldz[i][j][m][0] * v[i][j][k - 1][0] +
...
#pragma omp flush(flag)
    ...
    for (j = jst; j <= jend; j++) {
      for (m = 0; m < 5; m++) {
        ...
        v[i][j][k][m] =
            v[i][j][k][m] - omega * (ldy[i][j][m][0] * v[i][j - 1][k][0] +
...
#pragma omp flush(flag)
    ...
    for (j = jend; j >= jst; j--) {
      for (m = 0; m < 5; m++) {
        tv[i][j][m] =
            tv[i][j][m] + omega * (udy[i][j][m][0] * v[i][j + 1][k][0] +
...
#pragma omp for
    for (j = jst; j <= jend; j++) {
      for (k = 1; k <= nz - 2; k++) {
        for (i = ist; i <= iend; i++) {
          for (m = 0; m < 5; m++) {
            frct[i][j][k][m] = frct[i][j][k][m] - tx2 * (flux[i + 1][j][k][m] -
                                                         flux[i - 1][j][k][m]);
          }
        }
        for (i = ist; i <= L2; i++) {
...
#pragma omp for
    for (i = ist; i <= iend; i++) {
      for (k = 1; k <= nz - 2; k++) {
        for (j = jst; j <= jend; j++) {
          for (m = 0; m < 5; m++) {
            frct[i][j][k][m] = frct[i][j][k][m] - ty2 * (flux[i][j + 1][k][m] -
                                                         flux[i][j - 1][k][m]);
          }
        }
        for (j = jst; j <= L2; j++) {
...
#pragma omp for
    for (i = ist; i <= iend; i++) {
      for (j = jst; j <= jend; j++) {
        ...
        for (k = 1; k <= nz - 2; k++) {
          for (m = 0; m < 5; m++) {
            frct[i][j][k][m] = frct[i][j][k][m] - tz2 * (flux[i][j][k + 1][m] -
                                                         flux[i][j][k - 1][m]);
          }
        }
        for (k = 1; k <= nz - 1; k++) {
  ...
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      ...
      d[i][j][1][1] =
          1.0 +
          dt * 2.0 *
              (tx1 * r43 * c34 * tmp1 + ty1 * c34 * tmp1 + tz1 * c34 * tmp1) +
      ...
      d[i][j][2][2] =
          1.0 +
          dt * 2.0 *
              (tx1 * c34 * tmp1 + ty1 * r43 * c34 * tmp1 + tz1 * c34 * tmp1) +
      ...
      d[i][j][3][3] =
          1.0 +
          dt * 2.0 *
              (tx1 * c34 * tmp1 + ty1 * c34 * tmp1 + tz1 * r43 * c34 * tmp1) +
      ...
      d[i][j][4][4] =
          1.0 +
          dt * 2.0 *
              (tx1 * c1345 * tmp1 + ty1 * c1345 * tmp1 + tz1 * c1345 * tmp1) +
...
#pragma omp for
    for (j = jst; j <= jend; j++) {
      for (k = 1; k <= nz - 2; k++) {
        for (i = ist; i <= iend; i++) {
          for (m = 0; m < 5; m++) {
            rsd[i][j][k][m] = rsd[i][j][k][m] - tx2 * (flux[i + 1][j][k][m] -
                                                       flux[i - 1][j][k][m]);
          }
        }
...
#pragma omp for
    for (i = ist; i <= iend; i++) {
      for (k = 1; k <= nz - 2; k++) {
        for (j = jst; j <= jend; j++) {
          for (m = 0; m < 5; m++) {
            rsd[i][j][k][m] = rsd[i][j][k][m] - ty2 * (flux[i][j + 1][k][m] -
                                                       flux[i][j - 1][k][m]);
          }
        }
...
#pragma omp for
    for (i = ist; i <= iend; i++) {
      for (j = jst; j <= jend; j++) {
        ...
        for (k = 1; k <= nz - 2; k++) {
          for (m = 0; m < 5; m++) {
            rsd[i][j][k][m] = rsd[i][j][k][m] - tz2 * (flux[i][j][k + 1][m] -
                                                       flux[i][j][k - 1][m]);
          }
        }
  ...
  *verified = TRUE;
  ...
  if (nx0 == 12 && ny0 == 12 && nz0 == 12 && itmax == 50) {
    *class = 'S';
    ...
    grid, c   after 50 time steps, with  DT = 5.0d-01
    --------------------------------------------------------------------*/
    ...
    *class = 'A';
    ...
    grid, c   after 250 time steps, with  DT = 2.0d+0.0
    --------------------------------------------------------------------*/
