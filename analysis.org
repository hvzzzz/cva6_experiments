...
# ==========================================
# print("\n")
print("\n*✅ Unix Benchmark Comparison: Baseline vs 1st Implementation*")
...
#+begin_src jupyter-python :var base_raw=baseline_unix :var new_raw=2nd_unix :colnames no :results output drawer :exports results
import pandas as pd
from IPython.display import display
...
# ==========================================
# print("\n")
print("\n*Unix Benchmark Comparison: Baseline vs 2nd Implementation*")
styled = (final_view.style
    .format({
        'Score_old': '{:,.0f}',
        'Score_new': '{:,.0f}',
    ...
    .bar(subset=['IPC_Diff'], color='#5fba7d', vmin=-0.1, vmax=0.1)
    .hide(axis='index')
    # .set_caption("✅ <b>UnixBench Comparison: Baseline vs 2nd Implementation</b>")
)
...
| Label             | Score_old  | Score_new  | Score_Diff | Score_Diff_% | IPC_Diff | IPC_Diff_% |
|-------------------+------------+------------+------------+--------------+----------+------------|
| Pipe_Throughput   | 8,840      | 13,081     | +4,241     | +47.98%      | +0.0548  | +39.03%    |
| Context_Switching | 856        | 1,216      | +360       | +42.06%      | +0.0189  | +16.17%    |
| Sys Mix           | 22,651     | 27,655     | +5,004     | +22.09%      | +0.0330  | +18.87%    |
| Sys Exec          | 20         | 23         | +3         | +15.00%      | +0.0130  | +10.92%    |
| Sys GetPID        | 205,011    | 229,367    | +24,356    | +11.88%      | +0.0234  | +10.36%    |
| Dhrystone         | 259,837    | 281,643    | +21,806    | +8.39%       | +0.0237  | +5.29%     |
| Whetstone         | 19         | 21         | +1         | +6.71%       | +0.0254  | +6.18%     |
| Arith Double      | 658,256    | 696,012    | +37,756    | +5.74%       | +0.0300  | +5.27%     |
| Arith Overhead    | 22,607,716 | 23,890,762 | +1,283,046 | +5.68%       | +0.0204  | +5.25%     |
| Arith Short       | 659,170    | 696,400    | +37,230    | +5.65%       | +0.0299  | +5.25%     |
| Arith Int         | 658,720    | 695,912    | +37,192    | +5.65%       | +0.0300  | +5.28%     |
| Arith Register    | 658,341    | 691,936    | +33,595    | +5.10%       | +0.0293  | +5.15%     |
:end:
...
** Compare
#+begin_src jupyter-python :var base_raw=baseline_npb :var new_raw=1st_npb :colnames no :results output drawer :exports results
import pandas as pd
from IPython.display import display
...
# ==========================================
# print("\n")
print("\n*✅ NPB Benchmark Comparison: Baseline vs 1st Implementation*")
styled = (final_view.style
    .format({
...
#+begin_src jupyter-python :var base_raw=baseline_npb :var new_raw=2nd_npb :colnames no :results output drawer :exports results
import pandas as pd
from IPython.display import display
...
# ==========================================
# print("\n")
print("\n*✅ NPB Benchmark Comparison: Baseline vs 2nd Implementation*")
styled = (final_view.style
    .format({
        'Score_old': '{:,.2f}',      # Changed to 2 decimals for Mop/s
        'Score_new': '{:,.2f}',
    ...
    .bar(subset=['IPC_Diff'], color='#5fba7d', vmin=-0.05, vmax=0.05)
    .hide(axis='index')
    # .set_caption("✅ <b>NPB Benchmark Comparison: Baseline vs 2nd Implementation</b>")
)
...
| Label | Score_old | Score_new | Score_Diff | Score_Diff_% | IPC_Diff | IPC_Diff_% |
|-------+-----------+-----------+------------+--------------+----------+------------|
| IS    | 0.45      | 0.56      | +0.11      | +24.44%      | +0.0417  | +19.57%    |
| MG    | 1.07      | 1.21      | +0.14      | +13.08%      | +0.0224  | +11.64%    |
| CG    | 0.78      | 0.88      | +0.10      | +12.82%      | +0.0195  | +11.65%    |
| SP    | 1.21      | 1.35      | +0.14      | +11.57%      | +0.0179  | +9.88%     |
| LU    | 2.89      | 3.21      | +0.32      | +11.07%      | +0.0212  | +9.23%     |
| FT    | 1.19      | 1.30      | +0.11      | +9.24%       | +0.0122  | +7.65%     |
| EP    | 0.12      | 0.13      | +0.01      | +8.33%       | +0.0285  | +9.60%     |
| BT    | 2.15      | 2.30      | +0.15      | +6.98%       | +0.0123  | +5.87%     |
:end:
