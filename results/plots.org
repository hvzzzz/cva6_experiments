...
def prettify_config(conf):
    ...
    # Safely parses both IC and DC sizes (handles optional 'k')
    m = re.search(r'IC(\d+)(k?)_DC(\d+)(k?)', str(conf), re.IGNORECASE)
    if m: 
        ic_val, ic_k, dc_val, dc_k = m.groups()
        ic_unit = "KB" if ic_k.lower() == 'k' else "B"
        dc_unit = "KB" if dc_k.lower() == 'k' else "B"
        return f"IC {ic_val}{ic_unit} / DC {dc_val}{dc_unit}"

    m = re.search(r'IC/DC\s*(\d+)(k?)', str(conf), re.IGNORECASE)
    if m: 
        unit = "KB" if m.group(2).lower() == 'k' else "B"
        return f"IC {m.group(1)}{unit} / DC {m.group(1)}{unit}"

    # Catch both "1w" and "IC1w_DC1w"
    m = re.search(r'IC(\d+)w_DC(\d+)w', str(conf), re.IGNORECASE)
    if m: return f"{m.group(1)}-Way Assoc"
...
def get_sort_value(config_str):
    ...
    # Try to parse the *already prettified* format (since agg_data uses pretty names)
    m = re.search(r'IC\s*(\d+)\s*(KB|B)\s*/\s*DC\s*(\d+)\s*(KB|B)', config_str, re.IGNORECASE)
    if m:
        ic_val = int(m.group(1)) * (1024 if m.group(2).upper() == 'KB' else 1)
        dc_val = int(m.group(3)) * (1024 if m.group(4).upper() == 'KB' else 1)
        return (ic_val, dc_val)

    # Try raw format "IC4k_DC128"
    m = re.search(r'IC(\d+)(k?)_DC(\d+)(k?)', config_str, re.IGNORECASE)
    if m:
        ic_val = int(m.group(1)) * (1024 if m.group(2).lower() == 'k' else 1)
        dc_val = int(m.group(3)) * (1024 if m.group(4).lower() == 'k' else 1)
        return (ic_val, dc_val)
        
    m = re.search(r'IC/DC\s*(\d+)(k?)', config_str, re.IGNORECASE)
    if m:
        val = int(m.group(1)) * (1024 if m.group(2).lower() == 'k' else 1)
        return (val, val)

    # Fallback for Associativity, BTB, etc.
    m = re.search(r'(\d+)', config_str)
    if m:
        is_kilo = bool(re.search(fr'{m.group(1)}\s*(k|kb)', config_str, re.IGNORECASE))
        val = int(m.group(1))
        return (val * 1024 if is_kilo else val, 0)
...
for bench in benchmarks:
    ...
    # UPDATE: Changed -1 to (-1, -1) to match tuple returns from get_sort_value
    subset['sort_val'] = subset.apply(
        lambda row: (-1, -1) if row['sweep_type'] == 'Baseline' else get_sort_value(row['config']),
        axis=1
    )
    subset = subset.sort_values(by=['sweep_type', 'sort_val'])
    ...
    agg_data['sort_val'] = agg_data.apply(
        lambda row: (-1, -1) if row['sweep_type'] == 'Baseline' else get_sort_value(row['pretty_config']),
        axis=1
    )
...
# --- 1. Setup ---
RESULTS_DIR = "/home/han4n/cva6_experiments/results"
PLOTS_DIR = os.path.join(RESULTS_DIR, "speedup_plot_cycles_all")
os.makedirs(PLOTS_DIR, exist_ok=True)
...
def prettify_config(conf):
    ...
    # Cache
    m = re.search(r'IC(\d+)k_DC(\d+)k', conf, re.IGNORECASE)
    if m: return f"{m.group(1)}KB"
    m = re.search(r'IC/DC (\d+)k', conf, re.IGNORECASE)
    if m: return f"{m.group(1)}KB"
    
    # Assoc
    m = re.search(r'IC(\d+)w_DC(\d+)w', conf, re.IGNORECASE)
    if m: return f"{m.group(1)}-Way"
    m = re.search(r'^(\d+)w$', conf, re.IGNORECASE)
    if m: return f"{m.group(1)}-Way"
...
# --- 4. Configuration ---
sweeps = [
    {"name": "Cache Size",    "file": "sweep_cache_total.csv", "ref_cfg": "IC4k_DC4k"}, 
    {"name": "Associativity", "file": "sweep_assoc_total.csv", "ref_cfg": "IC4w_DC4w"},
    {"name": "BTB Size",      "file": "sweep_btb_total.csv",   "ref_cfg": "BTB16"},
    {"name": "BHT Size",      "file": "sweep_bht_total.csv",   "ref_cfg": "BHT16"},
]

print("Generating Percentage Relative Plots (Fixed Legends & Colors)...")
...
for sweep in sweeps:
    ...
    # Clean Data
    cols_to_numeric = ['score', 'cycles']
    for col in cols_to_numeric:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')
    
    df = df.dropna(subset=['score', 'cycles'])
    df = df[(df['score'] > 0) & (df['cycles'] > 0)]

    if df.empty:
        print(f"Skipping {sweep['name']} (No valid data)")
        continue

    # --- EXTRACT BASELINE ---
    base_rows = df[df['config'] == sweep['ref_cfg']]
    
    # Fallback logic
    if base_rows.empty:
        old_ref = ""
        if sweep['name'] == "Cache Size": old_ref = "IC/DC 4k"
        elif sweep['name'] == "Associativity": old_ref = "4w"
        if old_ref:
            base_rows = df[df['config'] == old_ref]
            if not base_rows.empty:
                print(f"Note: Using fallback baseline '{old_ref}' for {sweep['name']}")
                sweep['ref_cfg'] = old_ref

    if base_rows.empty:
        print(f"Warning: Baseline '{sweep['ref_cfg']}' not found.")
        continue

    # Mean of baseline
    base_agg = base_rows.groupby(['suite', 'name'])[['score', 'cycles']].mean().reset_index()
    base_clean = base_agg.rename(columns={'score': 'base_score', 'cycles': 'base_cycles'})

    # --- MERGE & CALCULATE ---
    df_merged = pd.merge(df, base_clean, on=['suite', 'name'], how='inner')

    # Math
    # df_merged['Perf_Pct'] = (df_merged['base_score'] / df_merged['score']) * 100
    # df_merged['Cycles_Pct'] = (df_merged['base_cycles'] / df_merged['cycles']) * 100
    df_merged['Perf_Pct'] = (df_merged['score']/df_merged['base_score']) * 100
    df_merged['Cycles_Pct'] = (df_merged['cycles']/df_merged['base_cycles'] ) * 100

    
    # Clean Infs
    df_merged = df_merged.replace([np.inf, -np.inf], np.nan).dropna(subset=['Perf_Pct', 'Cycles_Pct'])
    ...
    for suite in df_merged['suite'].unique():
        ...
        # 1. BARS (Speedup)
        # errorbar=('pi', 100) -> Whiskers from Min to Max
        bar_plot = sns.barplot(
            data=subset,
            x="name",
            y="Perf_Pct",
        ...
        # 2. POINTS (Cycle Usage)
        # CHANGED COLOR TO RED ('tab:red') for visibility
        CYCLE_COLOR = 'tab:red' 
        ...
        # Safe Limits
        all_vals = pd.concat([subset['Perf_Pct'], subset['Cycles_Pct']])
        all_vals = all_vals.dropna()
        if not all_vals.empty:
            y_max = all_vals.max()
            if np.isfinite(y_max):
                plt.ylim(0, max(110, y_max + 15))
            else:
                plt.ylim(0, 150)
        else:
            plt.ylim(0, 150)
        ...
        # Labels
        plt.title(f"{suite} Suite: Sensitivity to {sweep['name']}", fontsize=18, pad=15)
        plt.ylabel(r"Speedup (\%) relative to Mean Baseline", fontsize=14)
        plt.xlabel("")
        plt.xticks(rotation=45, ha='right', rotation_mode='anchor', fontsize=11)

        # --- FIXING THE LEGEND ---
        # 1. Extract Handles and Labels
        handles, labels = bar_plot.get_legend_handles_labels()
        
        # 2. Filter handles to find ONLY Rectangles (Bars)
        #    Seaborn sometimes adds Line2D objects for error bars into the legend handles list.
        #    We want to associate the unique labels with their colored Rectangles.
        unique_labels = subset['Config Label'].unique()
        
        # We manually build the list of patches matching the unique config labels
        bar_handles = []
        for i, label in enumerate(unique_labels):
            # Create a proxy Patch with the correct color from the palette
            color = current_palette[i % len(current_palette)]
            patch = mpatches.Patch(facecolor=color, edgecolor='black', label=label)
            bar_handles.append(patch)

        # 3. Create the Proxy Artist for "Cycle Usage"
        diamond_proxy = mlines.Line2D(
            [], [], 
            color=CYCLE_COLOR, 
            marker='D', 
            linestyle='-', 
            linewidth=1.5, 
            markersize=6, 
            label='Cycle Usage (Min/Mean/Max)'
        )
        
        # 4. Combine
        final_handles = bar_handles + [diamond_proxy]
        ...
        plt.figtext(
            0.5, 0.01,
            "Bars: Speedup | Red Diamonds: Cycle Usage | Error Bars: Min/Max Range",
            ha="center", fontsize=10, style='italic'
        )
...
#+RESULTS:
:results:
#+begin_exampleGenerating Percentage Relative Plots (Fixed Legends & Colors)...
Done! Plots saved to: /home/han4n/cva6_experiments/results/speedup_plot_cycles_all
#+end_example
:end:
...
# --- 1. Setup ---
RESULTS_DIR = "/home/han4n/cva6_experiments/results"
PLOTS_DIR = os.path.join(RESULTS_DIR, "speedup_plot_ipc_all")
os.makedirs(PLOTS_DIR, exist_ok=True)
...
def prettify_config(conf):
    ...
    # Cache - Always output explicit IC / DC format
    m = re.search(r'IC(\d+)(k?)_DC(\d+)(k?)', conf, re.IGNORECASE)
    if m: 
        ic_val, ic_k, dc_val, dc_k = m.groups()
        ic_unit = "KB" if ic_k.lower() == 'k' else "B"
        dc_unit = "KB" if dc_k.lower() == 'k' else "B"
    ...
    m = re.search(r'IC/DC\s*(\d+)(k?)', conf, re.IGNORECASE)
    if m: 
        unit = "KB" if m.group(2).lower() == 'k' else "B"
        return f"IC {m.group(1)}{unit} / DC {m.group(1)}{unit}"
...
def get_sort_value(config_str):
    ...
    # Return a tuple: (IC_size, DC_size) to sort by IC first, then DC
    m = re.search(r'IC(\d+)(k?)_DC(\d+)(k?)', config_str, re.IGNORECASE)
    if m:
        ic_val = int(m.group(1)) * (1024 if m.group(2).lower() == 'k' else 1)
        dc_val = int(m.group(3)) * (1024 if m.group(4).lower() == 'k' else 1)
    ...
    # Fallback for Associativity, BTB, etc. (returns tuple to keep sorting logic consistent)
    m = re.search(r'(\d+)(k?)', config_str, re.IGNORECASE)
    if m: 
        val = int(m.group(1))
        is_kilo = m.group(2).lower() == 'k'
        return (val * 1024 if is_kilo else val, 0)
...
print("Generating IPC Relative Plots (Baseline / Current)...")
...
for sweep in sweeps:
    ...
    # Clean Data
    cols_to_numeric = ['score', 'ipc'] 
    for col in cols_to_numeric:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')
    
    df = df.dropna(subset=['score', 'ipc'])
    df = df[(df['score'] > 0) & (df['ipc'] > 0)]
    ...
    # Mean of baseline
    base_agg = base_rows.groupby(['suite', 'name'])[['score', 'ipc']].mean().reset_index()
    base_clean = base_agg.rename(columns={'score': 'base_score', 'ipc': 'base_ipc'})
    ...
    # Math
    df_merged['Perf_Pct'] = (df_merged['score']/df_merged['base_score']) * 100   
    # df_merged['IPC_Pct'] = (df_merged['base_ipc'] / df_merged['ipc']) * 100
    df_merged['IPC_Pct'] = (df_merged['ipc'] / df_merged['base_ipc']) * 100
    # Clean Infs
    df_merged = df_merged.replace([np.inf, -np.inf], np.nan).dropna(subset=['Perf_Pct', 'IPC_Pct'])
    ...
    for suite in df_merged['suite'].unique():
        ...
        # Safe Limits
        all_vals = pd.concat([subset['Perf_Pct'], subset['IPC_Pct']])
        all_vals = all_vals.dropna()
        if not all_vals.empty:
            y_max = all_vals.max()
            if np.isfinite(y_max):
        ...
        # Labels
        plt.title(f"{suite} Suite: Sensitivity to {sweep['name']}", fontsize=18, pad=15)
        plt.ylabel(r"Rel. Performance vs Baseline (\%)", fontsize=14)
        plt.xlabel("")
        plt.xticks(rotation=45, ha='right', rotation_mode='anchor', fontsize=11)
        ...
        plt.figtext(
            0.5, 0.01,
            r"Bars: Score Speedup | Red Line: IPC Ratio ($Current/Baseline$) | Error Bars: Min/Max",
            ha="center", fontsize=10, style='italic'
        )
...
#+RESULTS:
:results:
#+begin_exampleGenerating IPC Relative Plots (Baseline / Current)...
Done! Plots saved to: /home/han4n/cva6_experiments/results/speedup_plot_ipc_all
#+end_example
:end:
...
# --- 1. Setup ---
RESULTS_DIR = "/home/han4n/cva6_experiments/results"
PLOTS_DIR = os.path.join(RESULTS_DIR, "speedup_plot_instructions_all")
os.makedirs(PLOTS_DIR, exist_ok=True)
...
print("Generating Relative Instruction Plots (Baseline / Current)...")
...
for sweep in sweeps:
    ...
    # Clean Data
    cols_to_numeric = ['score', 'instructions'] 
    for col in cols_to_numeric:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')
    
    df = df.dropna(subset=['score', 'instructions'])
    df = df[(df['score'] > 0) & (df['instructions'] > 0)]
    ...
    # Mean of baseline
    base_agg = base_rows.groupby(['suite', 'name'])[['score', 'instructions']].mean().reset_index()
    base_clean = base_agg.rename(columns={'score': 'base_score', 'instructions': 'base_inst'})
    ...
    # Instruction Count (Baseline / Current)
    # UPDATED FORMULA
    df_merged['Inst_Pct'] = (df_merged['base_inst'] / df_merged['instructions']) * 100

    # Clean Infs
    df_merged = df_merged.replace([np.inf, -np.inf], np.nan).dropna(subset=['Perf_Pct', 'Inst_Pct'])
    ...
    for suite in df_merged['suite'].unique():
        ...
        # 2. POINTS (Instructions) - DASHED LINES
        INST_COLOR = 'tab:blue' 
        ...
        sns.pointplot(
            ...
            y="Inst_Pct",
            hue="Config Label",
            palette=[INST_COLOR] * n_configs,
            dodge=0.8,
            markers='s',
            scale=0.6,
            join=False,       # Enable connecting lines
            # linestyles='--', # Make them dashed
            errorbar=('pi', 100),
            capsize=0.05,
            legend=False,
            ax=bar_plot
        ...
        # Safe Limits
        all_vals = pd.concat([subset['Perf_Pct'], subset['Inst_Pct']])
        all_vals = all_vals.dropna()
        if not all_vals.empty:
            y_max = all_vals.max()
            if np.isfinite(y_max):
        ...
        # Proxy Artist for Instructions
        inst_proxy = mlines.Line2D(
            [], [], 
            color=INST_COLOR, 
            marker='s', 
            linestyle='--', 
            linewidth=1.5, 
            markersize=6, 
            label='Instructions Ratio (Baseline/Current)'
        )
        
        final_handles = bar_handles + [inst_proxy]
        ...
        plt.figtext(
            0.5, 0.01,
            r"Bars: Score Speedup | Blue Dashed Line: Instruction Ratio ($Baseline/Current$) | Error Bars: Min/Max",
            ha="center", fontsize=10, style='italic'
        )
...
#+RESULTS:
:results:
#+begin_exampleGenerating Relative Instruction Plots (Baseline / Current)...
Done! Plots saved to: /home/han4n/cva6_experiments/results/speedup_plot_instructions_all
#+end_example
:end:
