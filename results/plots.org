...
#+RESULTS:
:results:
#+begin_exampleGenerated: utilization_plots_auto_final/sweep_cache_LUT.png
Generated: utilization_plots_auto_final/sweep_cache_FF.png
Generated: utilization_plots_auto_final/sweep_cache_BRAM.png
Generated: utilization_plots_auto_final/sweep_cache_DSP.png
...
def plot_3d_stacked_cache_sweep(df_filtered, filename_prefix):
    ...
    X: Instruction Cache
    Y: Data Cache
    Z: Metric Count (Stacked by Stage)
    """
    if df_filtered.empty:
        return
    ...
    # Define Stages and Colors (Same as 2D plots)
    stages = [
        ('S1_Fetch', 'Fetch', '#4e79a7'),        # Blue
        ('S2_Decode', 'Decode', '#f28e2b'),      # Orange
        ('S3_Issue_Net', 'Issue', '#e15759'),    # Red
    ...
    # Map sizes to integer indices (0, 1, 2...)
    ...
    for metric in metrics:
        ...
        # To calculate the Z-limit for scaling, we need the max total height
        stage_cols = [f"{s[0]}_{metric}" for s in stages]
        max_height = df_filtered[stage_cols].sum(axis=1).max()

        # Iterate through every configuration (row)
        for _, row in df_filtered.iterrows():
            ...
            # Start stacking from Z=0 for this bar
            current_z = 0
            ...
            for stage_code, stage_name, stage_color in stages:
                ...
                if pd.notna(height) and height > 0:
                    # Draw bar segment
                    # x, y, z, dx, dy, dz
                    ax.bar3d(x_idx - 0.2,  # X pos (centered slightly)
                             y_idx - 0.2,  # Y pos
                             current_z,    # Z start
                             0.4,          # Width
                             0.4,          # Depth
        ...
        ax.set_xlabel(r'\textbf{Instruction Cache}', labelpad=15, fontsize=12)
        ax.set_ylabel(r'\textbf{Data Cache}', labelpad=15, fontsize=12)
        ax.set_zlabel(rf'\textbf{{{metric} Utilization (Count)}}', labelpad=10, fontsize=12)
        
        ax.set_title(rf'\textbf{{Cache Sweep: {metric} Distribution}}', fontsize=16, pad=20)
        
        if max_height > 0:
            ax.set_zlim(0, max_height * 1.1)

        # --- LEGEND ---
        # Create proxy artists for the legend (since bar3d doesn't support labels directly)
        legend_handles = [mpatches.Patch(color=color, label=name) for _, name, color in stages]
        
        # Place legend outside to prevent cluttering the 3D view
        ax.legend(handles=legend_handles[::-1], # Reverse to match visual stack order if desired
...
def plot_split_metric(df_filtered, x_col, x_label, title_prefix, filename_prefix):
    """
    Standard 2D Stacked Bar Plot (Unchanged for other sweeps)
    """
    if df_filtered.empty:
        return
    ...
    for metric in metrics:
        ...
        for s_code, s_name, s_color in stages:
            ...
            for i, rect in enumerate(bars):
                ...
                # if pct > 20:
                #     # label_text = rf"\textbf{{{val_str}}} \\ ({int(pct)}\%)"
                #     label_text = rf"\shortstack[c]{{\textbf{{{val_str}}} \\ ({int(pct)}\%)}}"
                #     font_size = 8
                # elif pct > 1.0:
                # # elif pct > 0.5:
                #     label_text = rf"\textbf{{{val_str}}}"
...
# We keep rows where Associativity, BTB, and BHT are baseline
s1 = df[
    (df['ICw_num'] == BASELINE['ICw_num']) &
    (df['DCw_num'] == BASELINE['DCw_num']) &
    (df['BTB_num'] == BASELINE['BTB_num']) &
...
# Call the NEW 3D plotting function for S1
...
# -----------------------------------------------------------------------------
INPUT_CSV = "cva6_experiments_stages_summary_final.csv"
OUTPUT_DIR = "utilization_plots_cache_2d"
...
# Baseline for filtering (Used to fix the "other" variable when sweeping)
BASELINE = {
    'IC_num': 4096,  'ICw_num': 4,
    'DC_num': 4096,  'DCw_num': 4,
    'BTB_num': 16,   'BHT_num': 16,   'RAS_num': 2
...
try:
    df = pd.read_csv(INPUT_CSV)
    # Parse size columns to numeric for sorting/filtering
    for col in ['IC', 'ICw', 'DC', 'DCw', 'BTB', 'BHT', 'RAS']:
        if col in df.columns:
            df[f'{col}_num'] = df[col].apply(parse_size)
except FileNotFoundError:
...
# -----------------------------------------------------------------------------
# 4. PLOTTING FUNCTION (2D Stacked)
# -----------------------------------------------------------------------------
def plot_split_metric(df_filtered, x_col, x_label, title_prefix, filename_prefix):
    """
    Generates 2D Stacked Bar plots for specific metrics.
    """
    if df_filtered.empty:
        print(f"Warning: No data found for {title_prefix}")
        return

    # Sort by the numeric value of the x-axis column
    df_plot = df_filtered.sort_values(f'{x_col}_num')
    labels = df_plot[x_col].astype(str).tolist()
    x = np.arange(len(labels))
    width = 0.65
    ...
    # ---------------------------------------------------------
    # CONFIG: Metrics to plot
    # Uncomment others if you want LUT/FF/DSP as well
    # ---------------------------------------------------------
    metrics = ['BRAM'] 
    # metrics = ['LUT', 'FF', 'BRAM', 'DSP'] 
    ...
    for metric in metrics:
        ...
        for s_code, s_name, s_color in stages:
            ...
            for i, rect in enumerate(bars):
                ...
                # Dynamic labeling logic to avoid clutter
                if pct > 5 and height > 1: 
                    label_text = rf"\shortstack[c]{{\textbf{{{val_str}}} \\ ({int(pct)}\%)}}"
                    font_size = 8
                elif pct > 1.0:
                    label_text = rf"\textbf{{{val_str}}}"
...
# Filter for the "Cache Sweep" context 
# (where Associativity, BTB, and BHT are fixed at baseline)
cache_sweep_base = df[
    (df['ICw_num'] == BASELINE['ICw_num']) &
    (df['DCw_num'] == BASELINE['DCw_num']) &
    (df['BTB_num'] == BASELINE['BTB_num']) &
...
# --- PLOT 1: Instruction Cache (IC) vs Count ---
# To see the effect of IC, we must fix DC to the baseline value
df_ic_sweep = cache_sweep_base[
    cache_sweep_base['DC_num'] == BASELINE['DC_num']
].copy()

plot_split_metric(
    df_ic_sweep, 
    x_col='IC', 
    x_label='Instruction Cache Size', 
    title_prefix='IC Size Sweep', 
    filename_prefix='sweep_IC_only'
)

# --- PLOT 2: Data Cache (DC) vs Count ---
# To see the effect of DC, we must fix IC to the baseline value
df_dc_sweep = cache_sweep_base[
    cache_sweep_base['IC_num'] == BASELINE['IC_num']
].copy()

plot_split_metric(
    df_dc_sweep, 
    x_col='DC', 
    x_label='Data Cache Size', 
    title_prefix='DC Size Sweep', 
    filename_prefix='sweep_DC_only'
)
#+end_src

#+RESULTS:
:results:
#+begin_exampleGenerated: utilization_plots_cache_2d/sweep_IC_only_BRAM.png
Generated: utilization_plots_cache_2d/sweep_DC_only_BRAM.png
#+end_example
:end:
...
# -----------------------------------------------------------------------------
INPUT_CSV = "cva6_experiments_stages_summary_final.csv"
OUTPUT_DIR = "utilization_plots_3d_rotated"
...
# -----------------------------------------------------------------------------
# 4. PLOTTING FUNCTION (Rotated Views)
# -----------------------------------------------------------------------------
def plot_3d_rotated_views(df_filtered, filename_prefix):
    if df_filtered.empty: return

    # We only care about BRAM as per previous context, but easy to add others
    metrics = ['BRAM'] 
    ...
    for metric in metrics:
        fig = plt.figure(figsize=(14, 10)) # Large canvas
        ax = fig.add_subplot(111, projection='3d')
        ...
        for _, row in df_filtered.iterrows():
            ...
            for stage_code, stage_name, stage_color in stages:
                ...
                height = row[col_name]
                if pd.notna(height) and height > 0:
                    ax.bar3d(x_idx - 0.25, y_idx - 0.25, current_z, 
                             0.5, 0.5, height, # Slightly wider bars
                             color=stage_color, shade=True)
                    current_z += height

        # --- LABELS ---
        ax.set_xticks(range(len(ic_sizes)))
        ax.set_xticklabels(ic_labels, fontsize=10, rotation=-15, ha='left')
        ax.set_yticks(range(len(dc_sizes)))
        ax.set_yticklabels(dc_labels, fontsize=10, rotation=15, ha='right')
        
        ax.set_xlabel(r'\textbf{Instruction Cache}', labelpad=15, fontsize=12)
        ax.set_ylabel(r'\textbf{Data Cache}', labelpad=15, fontsize=12)
        ax.set_zlabel(rf'\textbf{{{metric} Count}}', labelpad=10, fontsize=12)
        ax.set_title(rf'\textbf{{{metric} Distribution}}', fontsize=18)

        # Legend
        legend_handles = [mpatches.Patch(color=color, label=name) for _, name, color in stages]
        ax.legend(handles=legend_handles[::-1], bbox_to_anchor=(1.05, 1), loc='upper left', title="Stage")

        # --- SAVE 3 DIFFERENT VIEWS ---

        # 1. Standard Isometric View
        ax.view_init(elev=30, azim=-60)
        plt.savefig(os.path.join(OUTPUT_DIR, f"{filename_prefix}_{metric}_iso.png"), bbox_inches='tight')
        print(f"Saved ISO view for {metric}")

        # 2. Instruction Cache View (Looking at the X-Z plane)
        # azim=-90 aligns the Y-axis (Data Cache) away from viewer, showing IC clearly
        ax.view_init(elev=10, azim=-90)
        plt.savefig(os.path.join(OUTPUT_DIR, f"{filename_prefix}_{metric}_side_IC.png"), bbox_inches='tight')
        print(f"Saved IC side view for {metric}")

        # 3. Data Cache View (Looking at the Y-Z plane)
        # azim=0 aligns the X-axis (Instruction Cache) away from viewer, showing DC clearly
        ax.view_init(elev=10, azim=0)
        plt.savefig(os.path.join(OUTPUT_DIR, f"{filename_prefix}_{metric}_side_DC.png"), bbox_inches='tight')
        print(f"Saved DC side view for {metric}")
...
plot_3d_rotated_views(s1, 'sweep_cache')
#+end_src

#+RESULTS:
:results:
#+begin_exampleSaved ISO view for BRAM
Saved IC side view for BRAM
Saved DC side view for BRAM
#+end_example
[[file:./.ob-jupyter/5c4f356c0fe7a193d6a0071d47b861e99cf7e116.png]]
:end:
...
#+RESULTS:
:results:
#+begin_exampleGenerated: soc_architecture_plots/soc_breakdown_pie_LUT.png
Generated: soc_architecture_plots/soc_breakdown_pie_FF.png
Generated: soc_architecture_plots/soc_scaling_trend_LUT.png
#+end_example
:end:
