...
for sweep in sweeps:
    ...
    # Math
    # Score: (Base_Score / Current_Score) * 100 -> Lower score is better (execution time?) 
    # OR if score is throughput (Higher is Better), usually Speedup = Current/Base.
    # Keeping your original logic for Perf_Pct:
    df_merged['Perf_Pct'] = (df_merged['base_score'] / df_merged['score']) * 100
    
    # IPC: (Baseline_IPC / Current_IPC) * 100
    # As requested:
