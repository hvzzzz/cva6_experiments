...
# --- 1. Setup ---
RESULTS_DIR = "/home/han4n/cva6_experiments/results"
PLOTS_DIR = os.path.join(RESULTS_DIR, "percentage_relative_plots_auto")
os.makedirs(PLOTS_DIR, exist_ok=True)
...
for sweep in sweeps:
    ...
    for suite in df_merged['suite'].unique():
        ...
        # Labels
        plt.title(f"{suite} Suite: Sensitivity to {sweep['name']}", fontsize=18, pad=15)
        plt.ylabel(r"Relative to Baseline (\%)", fontsize=14)
        plt.xlabel("")
        plt.xticks(rotation=45, ha='right', rotation_mode='anchor', fontsize=11)
        ...
        plt.tight_layout()
        filename = f"percent_{sweep['name'].replace(' ', '_')}_{suite}.png".lower()
        plt.savefig(os.path.join(PLOTS_DIR, filename), bbox_inches='tight')
        plt.close()
...
Done! Plots saved to: /home/han4n/cva6_experiments/results/percentage_relative_plots_auto
#+end_example
:end:
...
# -----------------------------------------------------------------------------
INPUT_CSV = "cva6_experiments_stages_summary.csv"
OUTPUT_DIR = "utilization_plots_auto"
...
#+RESULTS:
:results:
#+begin_exampleGenerated: utilization_plots_auto/sweep_cache_LUT.png
Generated: utilization_plots_auto/sweep_cache_FF.png
Generated: utilization_plots_auto/sweep_cache_BRAM.png
Generated: utilization_plots_auto/sweep_cache_DSP.png
Generated: utilization_plots_auto/sweep_assoc_LUT.png
Generated: utilization_plots_auto/sweep_assoc_FF.png
Generated: utilization_plots_auto/sweep_assoc_BRAM.png
Generated: utilization_plots_auto/sweep_assoc_DSP.png
Generated: utilization_plots_auto/sweep_btb_LUT.png
Generated: utilization_plots_auto/sweep_btb_FF.png
Generated: utilization_plots_auto/sweep_btb_BRAM.png
Generated: utilization_plots_auto/sweep_btb_DSP.png
Generated: utilization_plots_auto/sweep_bht_LUT.png
Generated: utilization_plots_auto/sweep_bht_FF.png
Generated: utilization_plots_auto/sweep_bht_BRAM.png
Generated: utilization_plots_auto/sweep_bht_DSP.png
#+end_example
:end:
...
# Baseline for filtering & Labeling
BASELINE = {
    'IC_num': 4096,  'ICw_num': 4,
    'DC_num': 4096,  'DCw_num': 4,
    'BTB_num': 16,   'BHT_num': 16,   'RAS_num': 2
...
def plot_split_metric(df_filtered, x_col, x_label, title_prefix, filename_prefix):
    ...
    labels = []
    baseline_key = f"{x_col}_num"
    baseline_val = BASELINE.get(baseline_key)

    for index, row in df_plot.iterrows():
        lbl = str(row[x_col])
        val = row[baseline_key]
        
        # Check if this row matches the global baseline value
        if val == baseline_val:
            # Append (Base) label using LaTeX bolding
            labels.append(rf"{lbl} \textbf{{(Base)}}")
        else:
            labels.append(lbl)
