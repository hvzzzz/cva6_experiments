...
output_csv = "./results/cva6_experiments_stages_summary.csv"
...
Success. Parsed 47 files.
Data saved to: ./results/cva6_experiments_stages_summary.csv

Preview of Logic and Memory breakdown:
    IC ICw  SoC_Total_LUT  S4_Execute_Net_LUT  S5_Memory_Total_LUT
6   2k  4w          47695               10492                15576
5   2k  4w          47714               10490                15590
13  4k  4w          47723               10486                15611
14  4k  4w          48199               10490                16059
12  4k  4w          47762               10484                15613
#+end_example
:end:
...
# -----------------------------------------------------------------------------
report_dir = "/home/han4n/cva6_experiments/bitstreams/reports/full"
output_csv = "./results/cva6_experiments_stages_summary_final.csv"
...
def calculate_stages_flat(raw_data, file_params):
    ...
    # KNOWN PERIPHERALS (The "Uncore" Bucket)
    # Add any other top-level modules found in your report here
    peripheral_modules = [
        "northbridge", 
        "axi_quad_spi_0", 
        "axi_uart16550_0", 
    ...
    for metric in metrics_list:
        ...
        soc_total = 0
        for key in ["SoC_wrapper", "SoC_i", "ariane_soc"]:
            if key in raw_data:
                soc_total = raw_data[key][metric]
                break
        row_data[f"SoC_Total_{metric}"] = soc_total

        # --- UNCORE (PERIPHERALS) ---
        uncore_val = 0
        for p_name in peripheral_modules:
            # We use loose matching because names often have suffixes like "_0"
            # But the dictionary keys are exact from the parsing function.
            # Based on your log, the names are fairly consistent.
            # We look for exact matches first, or partials if needed.
            # Your parsing script gets exact names like "axi_quad_spi_0" or "SoC_axi_quad_spi_0_0"
            # The parser extracts the "Instance" column. 
            # In your log: Instance="axi_quad_spi_0", Module="SoC_axi_quad_spi_0_0"
            # Your parser keys off Instance.
            uncore_val += get_val(p_name, metric)
        
        row_data[f"Uncore_{metric}"] = uncore_val

        # --- PIPELINE STAGES ---
        
        # 1. FETCH
        # Logic (Frontend) + Memory (I-Cache)
        fetch_val = get_val("i_frontend", metric) + get_val("i_cva6_icache", metric)
        ...
        # 3. ISSUE
        # Issue stage usually does NOT contain scoreboard in hierarchy
        issue_net = get_val("issue_stage_i", metric)

        # 5. MEMORY
        # LSU + D-Cache
        # FIX: Do NOT add mmu_val here, it is already inside lsu_i
        lsu_val = get_val("lsu_i", metric)
        ...
        # 4. EXECUTE
        # Execute usually DOES contain LSU in hierarchy, so we subtract
        ex_total = get_val("ex_stage_i", metric)
        execute_net = max(0, ex_total - lsu_val)
        ...
        # --- INTEGRITY CHECKS ---
        sum_stages = (fetch_val + decode_val + issue_net + 
                      execute_net + memory_val + commit_val)
        
        # Delta = SoC - (Core + Uncore)
        # Ideally this is 0. If positive, it's just glue logic (clock wizards, resets, etc)
        remainder = soc_total - (sum_stages + uncore_val)
        
        row_data[f"Sum_Stages_{metric}"] = sum_stages
        row_data[f"Remainder_{metric}"] = remainder
...
if all_rows:
    ...
    df.to_csv(output_csv, index=False)
    print(f"\nSuccess. Parsed {len(all_rows)} files.")
    print(f"Data saved to: {output_csv}")

    # -----------------------------------------------------------------------------
    # 5. NEW INTEGRITY REPORT
    # -----------------------------------------------------------------------------
    print("\n" + "="*60)
    print("FINAL INTEGRITY CHECK (SoC vs Core + Uncore)")
    print("="*60)
    
    for metric in ["LUT", "FF", "BRAM", "DSP"]:
        rem_col = f"Remainder_{metric}"
        # We allow a small tolerance for glue logic/buffers not in main modules
        # For BRAM/DSP, tolerance should be 0.
        tolerance = 100 if metric in ["LUT", "FF"] else 0
        
        errors = df[abs(df[rem_col]) > tolerance]
        
        if errors.empty:
            print(f"[{metric}] Status: PERFECT MATCH")
        else:
            print(f"[{metric}] Status: Small Mismatch (Likely Glue Logic)")
            print(f"    Avg Unaccounted: {errors[rem_col].mean():.2f}")
            if metric == "BRAM":
                print("    NOTE: If BRAM Remainder is 0, the 4 BRAMs are now correctly in Uncore.")
            print("-" * 60)
else:
    print("No matching files found.")
#+end_src
...
Success. Parsed 51 files.
Data saved to: ./results/cva6_experiments_stages_summary_final.csv

============================================================
FINAL INTEGRITY CHECK (SoC vs Core + Uncore)
============================================================
[LUT] Status: Small Mismatch (Likely Glue Logic)
    Avg Unaccounted: 2520.35
------------------------------------------------------------
[FF] Status: Small Mismatch (Likely Glue Logic)
    Avg Unaccounted: 2084.90
------------------------------------------------------------
[BRAM] Status: Small Mismatch (Likely Glue Logic)
    Avg Unaccounted: -12.00
    NOTE: If BRAM Remainder is 0, the 4 BRAMs are now correctly in Uncore.
------------------------------------------------------------
[DSP] Status: PERFECT MATCH
#+end_example
:end:
...
# -----------------------------------------------------------------------------
report_dir = "/home/han4n/cva6_experiments/bitstreams/reports/full"
output_csv = "./results/cva6_uncore_breakdown.csv"
...
# -----------------------------------------------------------------------------
def calculate_breakdown(raw_data, file_params):
    def get_val(name, metric):
        return raw_data.get(name, {}).get(metric, 0)

    row = file_params.copy()
    metrics = ["LUT", "FF", "BRAM", "DSP"]

    # Defined Peripherals based on your Vivado Log
    peripherals = {
        "Northbridge": "northbridge",
        "Peripherals_Subsystem": "ariane_peripherals_0", # Contains PLIC, Timer
        "SPI_Controller": "axi_quad_spi_0",
        "UART": "axi_uart16550_0",
        "Memory_Interconnect": "axi_mem_intercon",
        "CLINT": "clint_0",
        "BootROM": "bootrom_wrapper_0"
    }
    ...
    for m in metrics:
        ...
        for key in ["SoC_wrapper", "SoC_i", "ariane_soc"]:
            if key in raw_data:
                soc_total = raw_data[key][m]
                break
        row[f"SoC_Total_{m}"] = soc_total

        # 2. CPU Core Total (Calculated using verified stage logic)
        # Fetch (Logic + ICache)
        fetch = get_val("i_frontend", m) + get_val("i_cva6_icache", m)
        # Decode
        decode = get_val("id_stage_i", m)
        # Issue (Scoreboard is sibling)
        issue = get_val("issue_stage_i", m)
        # Memory (LSU + DCache, MMU is inside LSU)
        lsu = get_val("lsu_i", m)
        dcache = get_val("i_wt_dcache", m)
        if dcache == 0: dcache = get_val("gen_cache_wt.i_cache_subsystem", m)
        memory = lsu + dcache
        # Execute (Ex_Total - LSU)
        ex_total = get_val("ex_stage_i", m)
        execute = max(0, ex_total - lsu)
        # Commit
        commit = get_val("i_scoreboard", m)
        
        core_total = fetch + decode + issue + execute + memory + commit
        row[f"CPU_Core_{m}"] = core_total

        # 3. Peripheral Breakdown
        uncore_sum = 0
        for friendly_name, module_name in peripherals.items():
            val = get_val(module_name, m)
            row[f"{friendly_name}_{m}"] = val
            uncore_sum += val
        
        row[f"Uncore_Total_{m}"] = uncore_sum

        # 4. Remainder (Glue Logic)
        # Logic: SoC - (Core + Uncore)
        # This should be very small (Just top-level wires/buffers)
        remainder = soc_total - (core_total + uncore_sum)
        row[f"Glue_Logic_{m}"] = remainder
...
# -----------------------------------------------------------------------------
all_rows = []
print(f"Reading logs from: {report_dir}")

files = [f for f in os.listdir(report_dir) if f.endswith(".rpt")]
for filename in sorted(files):
    match = file_regex.search(filename)
    if match:
        path = os.path.join(report_dir, filename)
        raw = parse_vivado_report(path)
        if raw:
            row = calculate_breakdown(raw, match.groupdict())
            all_rows.append(row)
...
if all_rows:
    ...
    # Sort by IC size
    if 'IC' in df.columns:
        df['sort_h'] = df['IC'].apply(lambda x: int(x.replace('k','')) if 'k' in x else int(x))
        df = df.sort_values('sort_h').drop(columns=['sort_h'])

    df.to_csv(output_csv, index=False)
    print(f"Success. Breakdown saved to: {output_csv}")

    # --- PRINT SUMMARY ---
    print("\n" + "="*80)
    print("UNCORE RESOURCE BREAKDOWN (Where the rest went)")
    print("="*80)
    
    # We take the mean of the first file configuration to show a representative breakdown
    # (Peripherals shouldn't change much between cache sweeps)
    example = df.iloc[0]
    
    for metric in ["LUT", "BRAM"]:
        print(f"\n--- {metric} Breakdown ---")
        print(f"{'Module':<30} | {'Count':<10} | {'% of SoC':<10}")
        print("-" * 55)
        
        soc = example[f"SoC_Total_{metric}"]
        if soc == 0: continue

        # CPU Core
        core = example[f"CPU_Core_{metric}"]
        print(f"{'CPU Core':<30} | {core:<10} | {core/soc*100:.1f}%")
        
        # Peripherals
        peri_keys = [c for c in df.columns if f"_{metric}" in c and c not in [
            f"SoC_Total_{metric}", f"CPU_Core_{metric}", f"Uncore_Total_{metric}", f"Glue_Logic_{metric}"
        ]]
        
        for key in peri_keys:
            # Extract friendly name
            name = key.replace(f"_{metric}", "")
            val = example[key]
            if val > 0:
                print(f"{name:<30} | {val:<10} | {val/soc*100:.1f}%")
        
        # Glue Logic
        glue = example[f"Glue_Logic_{metric}"]
        print(f"{'Glue Logic / Other':<30} | {glue:<10} | {glue/soc*100:.1f}%")
        print("-" * 55)
        print(f"{'TOTAL':<30} | {soc:<10} | 100.0%")
...
#+RESULTS:
:results:
#+begin_exampleReading logs from: /home/han4n/cva6_experiments/bitstreams/reports/full
Success. Breakdown saved to: ./results/cva6_uncore_breakdown.csv

================================================================================
UNCORE RESOURCE BREAKDOWN (Where the rest went)
================================================================================
...
-------------------------------------------------------
CPU Core                       | 40018      | 83.9%
Northbridge                    | 3641       | 7.6%
Peripherals_Subsystem          | 1875       | 3.9%
SPI_Controller                 | 560        | 1.2%
UART                           | 345        | 0.7%
Memory_Interconnect            | 361        | 0.8%
CLINT                          | 183        | 0.4%
BootROM                        | 65         | 0.1%
Glue Logic / Other             | 647        | 1.4%
-------------------------------------------------------
TOTAL                          | 47695      | 100.0%
...
INPUT_CSV = "./results/cva6_experiments_stages_summary.csv"
...
#+begin_example
#+end_example
[[file:./.ob-jupyter/f199c12527dd58b9e6a9a26d91a643fc456db4c5.png]]
#+begin_example
Generated plot_cache.png
#+end_example
[[file:./.ob-jupyter/bd394e3cc81a550dfa68840f61a5c0045668eaae.png]]
#+begin_example
Generated plot_assoc.png
#+end_example
[[file:./.ob-jupyter/568fd45312f59232d71e6f8f93bd02fdc7dba62a.png]]
#+begin_example
Generated plot_btb.png
#+end_example
[[file:./.ob-jupyter/3e559af374e6c252b30606f8524f480172b1d2ce.png]]
#+begin_example
